...
`,we=`<svg xmlns="http://www.w3.org/2000/svg"
  viewBox="0 0 100 200" style="display:block;width:100%;height:100%">
  <!-- tail (behind body) -->
...
~D,~D,~D,~,~B,~F
~C,~C,~C,~H,~B,~j
~k,~k,~,~,~B,~j`}],F=Ae.map(({name:e,csv:n})=>le(e.toLowerCase().replace(/\s+/g,"-"),e,n));if(F.length===0)throw new Error("No levels found in levels.generated.json");const I=document.getElementById("game-container"),Me=document.getElementById("restart-btn");let W=0,R=null;function G(e){W=e;const n=F[e];R==null||R();const t=ie(n),a=document.getElementById("level-label");a&&(a.textContent=/^\d+$/.test(n.label)?`Level ${n.label}`:n.label),Z(I);const i=n.riverCells!==void 0&&n.riverCells.size===n.rows*n.cols;document.body.style.background=i?"#3a7bbf":"#4a7a30";const s=n.rows-(n.bleedTop??0)-(n.bleedBottom??0);ye(I,n.rows,n.cols,n.riverCells,s),xe(I,t),q(0),R=ke(I,t,()=>{const c=new Audio(Ee);$e(I),c.play(),c.addEventListener("ended",()=>{Ce(I),W+1<F.length?oe(W+1):be(I,"You finished the game!<br>Ask your dad for more levels :)")})})}const Q="./".replace(/\/$/,"");function ee(){const e=window.location.pathname.match(/\/(\d+)$/);if(e){const n=parseInt(e[1],10);if(n>=1&&n<=F.length)return n-1}return 0}function oe(e){history.pushState({levelIndex:e},"",`${Q}/${e+1}`),G(e)}window.addEventListener("popstate",e=>{var n;G(((n=e.state)==null?void 0:n.levelIndex)??ee())});Me.addEventListener("click",()=>{Z(I),oe(0)});const _=ee();history.replaceState({levelIndex:_},"",`${Q}/${_+1}`);G(_);
